...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    ...
    if (sender == self.saving) {
        if (segmentedStaffBen.selectedSegmentIndex == 0) {
            ...
            PFObject *nuevo = [PFObject objectWithClassName:@"Staff"];
            nuevo[@"nombre"] = self.nombreAlumno.text;
            nuevo[@"proyecto"] = self.stringAsistenciaProyecto;
            nuevo[@"apellido"] = self.tfApellido.text;
            nuevo[@"faltas"] = @0;
            nuevo[@"presente"] = @NO;
            nuevo[@"matricula"] = self.tfMatricula.text;
            
            PFObject *nuevo2 = [PFObject objectWithClassName:@"User"];
            nuevo2[@"username"] = self.nombreAlumno.text;
            nuevo2[@"password"] = self.tfMatricula.text;
            nuevo2[@"nombre"] = @"Staff";

            [nuevo saveInBackground];
            [nuevo2 saveInBackground];
...
@synthesize segmentedStaffBen;
- (IBAction)segmentedAction:(id)sender {
    if (segmentedStaffBen.selectedSegmentIndex == 1){
        self.matriculaAlumno.hidden = true;
        self.tfMatricula.hidden = true;
    }
    else {
        self.matriculaAlumno.hidden = false;
        self.tfMatricula.hidden = false;
    }
